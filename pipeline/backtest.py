...
def _resolve_macro_inputs(
    ...
    # 当前 quant.macro 仍使用 hy_oas_df 这个输入位。
    # 如果你未来把真正的 HY_OAS parquet 拉下来了，这里会优先使用。
...
def run_backtest_pipeline(
    *,
    ...
    logger.info("========== Start backtest experiment ==========")
    logger.info(
    ...
    logger.info("Signals computed | count=%s", len(action_signals))

    # =========================
    # 4. Merge Signals + Raw Bars
    # =========================
    logger.info("Merging action signals into feature frame...")
    final_df = feature_df.copy()
    final_df["action_signal"] = action_signals
    ...
    logger.info("========== Backtest experiment finished ==========")
    return final_df


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run offline backtest experiment pipeline")
