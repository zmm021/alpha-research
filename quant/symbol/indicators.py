...
def compute_symbol_indicators(
    ...
    # ===== Moving averages =====
    ma20 = close.rolling(window=ma_short_window, min_periods=ma_short_window).mean()
    ma50 = close.rolling(window=ma_long_window, min_periods=ma_long_window).mean()
    ...
    # ===== Position =====
    rolling_high = df[Fields.HIGH].rolling(window=high_window, min_periods=high_window).max()
